...
NOTE: This vendored runtime is used by the parent BDI-agent pipeline repository.
That repository tracks its full benchmark query records in
`src/benchmark_data/benchmark_queries.json` and the corresponding validated
LTLf temporal-specification records in `src/benchmark_data/queries_LTLf.json`;
generated run outputs remain local-only.
