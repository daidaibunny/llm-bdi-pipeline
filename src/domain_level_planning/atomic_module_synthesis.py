		...
		for requirement in state.requirements:
			...
			for producer, producer_effect in _producer_effects(
				...
				if step_signature in state.used_step_signatures:
					continue
				producer_maps = _producer_maps_for_regression_requirement(
					action=producer,
					effect=producer_effect,
					requirement=requirement,
...
def _producer_maps_for_regression_requirement(
	*,
	action: _ParsedAction,
	effect: PDDLLiteralSchema,
	requirement: PDDLLiteralSchema,
	state_requirements: Sequence[PDDLLiteralSchema],
	...
	for raw_argument, required_argument in zip(effect.arguments, requirement.arguments):
		if raw_argument not in parameter_names:
			if raw_argument != required_argument:
				return ()
			continue
		if raw_argument in base_map and base_map[raw_argument] != required_argument:
			return ()
		base_map[raw_argument] = required_argument

	partial_maps: list[dict[str, str]] = [base_map]
	positive_requirements = tuple(
		item for item in state_requirements if item.is_positive
	)
	for precondition in action.preconditions:
		if not precondition.is_positive:
			continue
		next_maps = list(partial_maps)
		for partial_map in partial_maps:
			for current_requirement in positive_requirements:
				extended = _extend_action_map_from_matching_literal(
					action_parameters=parameter_names,
					literal=precondition,
					requirement=current_requirement,
					variable_map=partial_map,
				)
				if extended is not None:
					next_maps.append(extended)
		partial_maps = _deduplicate_variable_maps(
			next_maps,
			parameter_order=action.parameters,
		)

	completed_maps = [
		_complete_variable_map(
			action.parameters,
			partial_map,
			avoid_variables=avoid_variables - set(partial_map.values()),
		)
		for partial_map in partial_maps
	]
	return tuple(
		variable_map
		for variable_map in _deduplicate_variable_maps(
			completed_maps,
			parameter_order=action.parameters,
		)
		if _same_literal(effect.mapped(variable_map), requirement)
	)


def _extend_action_map_from_matching_literal(
	*,
	action_parameters: set[str],
	literal: PDDLLiteralSchema,
	requirement: PDDLLiteralSchema,
	variable_map: Mapping[str, str],
) -> dict[str, str] | None:
	if (
		literal.predicate != requirement.predicate
		or literal.is_positive != requirement.is_positive
		or len(literal.arguments) != len(requirement.arguments)
	):
		return None
	extended = dict(variable_map)
	for raw_argument, required_argument in zip(
		literal.arguments,
		requirement.arguments,
	):
		if raw_argument not in action_parameters:
			if raw_argument != required_argument:
				return None
			continue
		bound = extended.get(raw_argument)
		if bound is not None and bound != required_argument:
			return None
		extended[raw_argument] = required_argument
	return extended


def _deduplicate_variable_maps(
	variable_maps: Sequence[Mapping[str, str]],
	*,
	...
	for variable_map in variable_maps:
		key = tuple(
			(parameter, variable_map[parameter])
			for parameter in parameter_order
			if parameter in variable_map
		)
		if key in seen:
			continue
		seen.add(key)
		deduplicated.append(dict(variable_map))
	return deduplicated
...
def _runtime_plan_priority(
	...
	*,
	...
	return (
		plan.trigger.symbol,
		_runtime_plan_semantic_priority(plan, rule_kind=rule_kind),
...
def _runtime_plan_semantic_priority(plan: AgentSpeakPlan, *, rule_kind: str) -> int:
	"""Prefer certified discharge and its preparation before resource debt."""

	has_resource_release = _plan_has_resource_release_certificate(plan)
	if rule_kind == "already_true":
		return 0
	if rule_kind == "producer_action_sequence" and has_resource_release:
		return 1
	if rule_kind == "prepare_public_precondition" and has_resource_release:
		return 2
	if rule_kind == "producer_action_sequence":
		return 3
	if rule_kind == "prepare_public_precondition":
		return 4
	return 9


def _plan_has_resource_release_certificate(plan: AgentSpeakPlan) -> bool:
	"""Return whether one branch belongs to a target-preserving discharge path."""

	for certificate in tuple(plan.binding_certificate or ()):
		for resource in tuple(certificate.get("resource_release_certificates") or ()):
			if (
				isinstance(resource, Mapping)
				and resource.get("certificate_kind")
				== "causal_resource_capacity_invariant_discharge"
				and resource.get("target_preserved", True)
			):
				return True
	return False
...
def _clingo_selector_program(
	*,
	...
	lines: list[str] = [
		...
		"ranking_scope(Root, Predicate, Relation) :- selected(Root), "
		"ranking_root(Root, Predicate, Relation).",
		"ranking_scope(Root, Callee, Relation) :- ranking_scope(Root, Caller, Relation), "
		"preparation_edge(Caller, Callee).",
		":- ranking_scope(Root, Predicate, Relation), selected(Branch), "
		"relation_increaser(Branch, Predicate, Relation), "
		"not ranking_safe(Root, Branch).",
		...
		"relational_covered(Capability) :- recursive_covered(Capability), "
		"relational_capability(Capability).",
	...
	for capability_index, candidate_indexes in enumerate(recursive_capability_groups):
		if any(
			_recursive_progress_certificate_from_plan(plans[candidate_index]) is not None
			for candidate_index in candidate_indexes
		):
			lines.append(f"relational_capability(r{capability_index}).")
		for candidate_index in candidate_indexes:
			lines.append(
	...
	relation_names = sorted(
		{
			certificate.relation_predicate
			for certificate in (
				_recursive_progress_certificate_from_plan(plan) for plan in plans
			)
			if certificate is not None
		},
	)
	relation_ids = {
		relation: f"rel{index}" for index, relation in enumerate(relation_names)
	}
	for index, plan in enumerate(plans):
		branch_id = branch_ids[index]
		...
		progress = _recursive_progress_certificate_from_plan(plan)
		if progress is not None:
			lines.append(
				f"ranking_root({branch_id}, {predicate_ids[plan.trigger.symbol]}, "
				f"{relation_ids[progress.relation_predicate]}).",
			)
	for candidate_index, candidate in enumerate(plans):
		candidate_id = branch_ids[candidate_index]
		for relation, relation_id in relation_ids.items():
			if not _plan_directly_adds_predicate(
				...
				continue
			lines.append(
				f"relation_increaser({candidate_id}, "
				f"{predicate_ids[candidate.trigger.symbol]}, {relation_id}).",
			)
			for root_index, root in enumerate(plans):
				progress = _recursive_progress_certificate_from_plan(root)
				if (
					progress is None
					or progress.relation_predicate != relation
					or progress.ranking_feature_kind
					!= "anchored_acyclic_relation_cone_count"
				):
					continue
				if _plan_preserves_anchored_relation_cone(
					candidate,
					recursive_plan=root,
					...
					lines.append(
						f"ranking_safe({branch_ids[root_index]}, {candidate_id}).",
					)
	...
	for recursive_index, recursive_plan in enumerate(plan_tuple):
		...
		for candidate_index, candidate in enumerate(plan_tuple):
			if candidate.trigger.symbol not in reachable_modules:
				continue
			if candidate.trigger.symbol != recursive_plan.trigger.symbol:
				# Cross-predicate threats are conditioned on the preparation edges
				# selected by Clingo, not on unused edges in the raw candidate graph.
				continue
			if _plan_directly_adds_predicate(
				...
				if (
					certificate.ranking_feature_kind
					== "anchored_acyclic_relation_cone_count"
					and _plan_preserves_anchored_relation_cone(
						candidate,
...
def _plan_preserves_anchored_relation_cone(
	plan: AgentSpeakPlan,
	*,
	recursive_plan: AgentSpeakPlan,
	certificate: _RecursiveProgressCertificate,
	actions_by_name: Mapping[str, _ParsedAction],
) -> bool:
	"""Prove that one finite branch adds the relation only away from the anchor."""

	if plan.trigger.symbol != recursive_plan.trigger.symbol:
		return False
	if len(plan.trigger.arguments) != len(recursive_plan.trigger.arguments):
		return False
	if len(certificate.anchor_arguments) != 1:
		return False
	anchor_argument = certificate.anchor_arguments[0]
	if anchor_argument not in recursive_plan.trigger.arguments:
		return False
	if anchor_argument not in certificate.relation_arguments:
		return False
	trigger_anchor_position = recursive_plan.trigger.arguments.index(anchor_argument)
	relation_anchor_position = certificate.relation_arguments.index(anchor_argument)
	branch_anchor = plan.trigger.arguments[trigger_anchor_position]
	final_relation_state: dict[tuple[str, ...], bool] = {}
	for step in tuple(plan.body or ()):
		if step.kind != "action":
			return False
		action = actions_by_name.get(step.symbol)
		...
		for effect in action.delete_effects:
			if effect.predicate != certificate.relation_predicate:
				continue
	...
	for arguments, is_added in final_relation_state.items():
		if not is_added:
			continue
		if relation_anchor_position >= len(arguments):
			return False
		added_anchor = arguments[relation_anchor_position]
		if not _contexts_prove_terms_distinct(
			added_anchor,
			branch_anchor,
