...
def render_main_table(result: Mapping[str, Any]) -> str:
	"""Render the compact main-paper table for per-seed held-out coverage."""

	protocol = dict(result["protocol"])
	domains = {row["domain"]: row for row in result["domains"]}
	seed_results = tuple(result["seed_results"])
	seed_count = len(seed_results)
	logistics = domains.get("logistics")
	depots = domains.get("depots")
	other_domains = [
		row for row in result["domains"] if row["domain"] not in {"logistics", "depots"}
	]
	other_total = sum(row["test_count"] for row in other_domains)
	lines = [
		"% Auto-generated by scripts/freeze_five_seed_full_compiler_results.py.",
		"\\begin{table}[t]",
		"\\centering",
		...
		(
			"\\caption{Full GP2PL atomic held-out success by independent evidence "
			f"seed. The other {len(other_domains)} domains complete for all seeds; "
			"the final row is mean $\\pm$ sample standard deviation in percentage "
			"points.}"
		),
		...
		"\\toprule",
		(
			f"Seed & Valid/{_format_int(protocol['case_count_per_seed'])} & "
			"Logistics & Depots & "
			f"Other {len(other_domains)} \\\\"
		),
		"\\midrule",
	]
	for index, seed_row in enumerate(seed_results):
		logistics_cell = _domain_count_cell(logistics, index)
		depots_cell = _domain_count_cell(depots, index)
		other_success = sum(row["success_counts"][index] for row in other_domains)
		lines.append(
			f"{seed_row['seed']} & {seed_row['success_count']}/"
			f"{protocol['case_count_per_seed']} & {logistics_cell} & {depots_cell} & "
			f"{other_success}/{other_total} \\\\"
		)
	lines.extend(
		[
			"\\midrule",
			(
				"Mean (\\%) & "
				f"{result['aggregate']['mean_success_rate'] * 100:.2f} $\\pm$ "
				f"{result['aggregate']['sample_sd_success_rate'] * 100:.2f} & "
				f"{_domain_rate_cell(logistics)} & {_domain_rate_cell(depots)} & "
				f"{_combined_domain_rate_cell(other_domains, seed_count)} \\\\"
			),
			"\\bottomrule",
			"\\end{tabular}",
			"\\end{table}",
...
def render_domain_table(result: Mapping[str, Any]) -> str:
	...
	lines = [
		...
		(
			...
			"MOOSE goal-order seed. Mean and sample standard deviation are success "
			"percentages across the five libraries; no evidence is pooled and no "
	...
	for row in result["domains"]:
		...
		lines.append(
			...
			f"{row['mean_success_rate'] * 100:.2f} & "
			f"{row['sample_sd_success_rate'] * 100:.2f} \\\\"
		)
...
def _domain_rate_cell(domain: Mapping[str, Any] | None) -> str:
	if domain is None:
		return "--"
	return (
		f"{domain['mean_success_rate'] * 100:.2f} $\\pm$ "
		f"{domain['sample_sd_success_rate'] * 100:.2f}"
	)


def _combined_domain_rate_cell(
	domains: Sequence[Mapping[str, Any]],
	seed_count: int,
) -> str:
	total = sum(row["test_count"] for row in domains)
	if total == 0:
		return "--"
	rates = [
		sum(row["success_counts"][index] for row in domains) / total
		for index in range(seed_count)
	]
	return f"{statistics.mean(rates) * 100:.2f} $\\pm$ {statistics.stdev(rates) * 100:.2f}"
