...
def render_domain_table(result: dict[str, Any]) -> str:
	...
	lines = [
		...
		"\\caption{Per-domain structure of the hash-locked seed-0 atomic libraries "
		"and conditional temporal-goal execution. Evidence and schema are certified "
		"candidate branches before joint selection. End-to-end counts a bound query "
		"only when Jason, neutral-goal VAL, gold-DFA acceptance, and predicted-DFA "
		"acceptance all succeed; runtime is the median wall-clock seconds per query.}",
		"\\label{tab:domain-results}",
		"\\begin{tabular}{lrrrrrrrr}",
		"\\toprule",
		"& \\multicolumn{2}{c}{Corpus} & \\multicolumn{4}{c}{Atomic library} "
		"& \\multicolumn{2}{c}{Temporal execution} \\\\",
		"\\cmidrule(lr){2-3} \\cmidrule(lr){4-7} \\cmidrule(lr){8-9}",
		"Domain & Train & Test & Evidence & Schema & Selected & KiB "
		"& End-to-end & Median s \\\\",
		"\\midrule",
	]
...
def render_profile_table(result: dict[str, Any]) -> str:
	...
	lines = [
		...
		"\\toprule",
		"Profile & Translation & \\multicolumn{6}{c}{Bound-query pipeline} "
