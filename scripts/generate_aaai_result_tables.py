...
def render_domain_table(result: dict[str, Any]) -> str:
	...
	for row in result["domains"]:
		...
		lines.append(
			...
			f"{row['train_count']}/{row['test_count']} & ok/ok & "
...
def render_profile_table(result: dict[str, Any]) -> str:
	...
	for row in result["profiles"]:
		lines.append(
			f"{_profile_label(row['profile'])} & {row['query_count']} & "
			f"{row['translation_count']}/{row['dfa_equivalent_count']} & "
			f"{row['controller_compiled_count']}/{row['jason_success_count']}/"
			f"{row['val_success_count']}/{row['gold_dfa_accept_count']}/"
			f"{row['prediction_dfa_accept_count']} & "
