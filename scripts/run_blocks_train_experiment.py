#!/usr/bin/env python3
"""
Run the reproducible Blocks IPC train-split domain-level library experiment.
"""
...
def main() -> None:
	parser = argparse.ArgumentParser(
		description="Run the Blocks IPC train-split lifted ASL library experiment.",
	)
	parser.add_argument(
		"--output",
		type=Path,
		default=Path("tmp/blocks-ipc-train-experiment/report.json"),
	...
	parser.add_argument(
		...
		type=int,
		default=1,
		help="Number of earliest Blocks IPC training problems used for synthesis.",
	)
	parser.add_argument(
		"--eval-count",
		type=int,
		default=20,
		help="Number of earliest Blocks IPC training problems used for evaluation.",
	)
	...
	problems = tuple(sorted((blocks_root / "train").glob("*.pddl")))
	...
	report = run_domain_level_experiment(
		experiment_name="blocks-ipc-train20",
