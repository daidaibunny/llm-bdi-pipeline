...
def build_comparison_dataset(
	*,
	...
	_validate_raw_moose_runs(raw_records)
	...
	challenge_count = int(challenge.get("case_count") or 0)
	challenge_success = int(challenge.get("success_count") or 0)
	if challenge_count <= 0 or challenge_success != challenge_count:
		raise ValueError(
			"challenge matrix is incomplete: "
			f"{challenge_success}/{challenge_count}",
		)
	return {
		"schema_version": 1,
...
def _aggregate_temporal(
	...
	result_maps = {
		variant: {
			str(record.get("sample_id") or ""): dict(record)
			for record in by_variant[variant].get("results") or ()
		}
