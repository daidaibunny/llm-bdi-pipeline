...
| N12 | Expand the matrix across all local PDDL domains and make long rows diagnostic-safe. | G6, G10, G11 | `paper-expanded-smoke` covers Blocksworld, Labworkflow, Transport, Satellite, and Marsrover; per-entry and per-problem timeouts prevent runaway rows; failures identify the next generic Layer B/C gaps. | Done for the refreshed diagnostic suite. Latest `paper-expanded-smoke-latest` summary after backend adapter changes: 12 succeeded rows, 1 failed diagnostic row; Blocksworld paper-profile rows are 20/20, 10/10, and 10/10 with `paper_profile_ready=true`; the no-Layer-C-ordering first20 ablation is 20/20 but explicitly filters 9 ordering rules; Transport train3 first10 is 10/10; Satellite train3 first10 is 10/10; Marsrover fails before library generation because bounded transition-system exploration exceeds 20000 states. |
...
| N24 | Decide Marsrover final treatment. | G6, G10 | Either repair Marsrover synthesis under fixed resource limits using validated trace/external evidence, or move it to a limitation-only matrix with the state-explosion diagnosis. | Decided for the current paper: Marsrover is a limitation diagnostic, not a main claim row. Latest final limitation matrix reports Marsrover `0/10` with `Training transition-system exploration exceeded 20000 states for problem roverprob3726.` Transport remains a secondary limitation/fragment row: initial-state fragment `10/10`, runtime planner `none`, but arbitrary reachable mid-state coverage is not claimed. |
