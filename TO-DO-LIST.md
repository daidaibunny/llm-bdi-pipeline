...
| G12 | Benchmark-class boundary | Done for current paper framing: the final paper now states support over problem classes `Q_D`, not loose domain buckets. The tracked taxonomy records how prior GP papers choose and count domains: MOOSE uses 8 classical ESHO domains plus 4 numeric domains and a 38-domain IPC goal-independence probe; learner-sketches uses 9 IPC-style domains; D2L uses 9 feature-definable problem classes; PG3 uses 6 PDDL domains; BFGP++ uses 13 program-synthesis domains. Our final target now follows that scale with three selected property-defined classes and exactly 9 selected standard domain names: Gripper, Ferry, Miconic; Spanner, Childsnack, Barman, Visitall, Delivery; and Blocksworld. TEG is a cross-cutting goal-specification layer, not a fourth domain class. | Next implementation expansion should work through the selected 9-domain target under the same strict ASL-library protocol. | Done |
| G13 | Registry-driven benchmark infrastructure | Done for achievement-goal infra. The single source of truth is now `src/benchmark_registry/achievement_goals`: it has class/domain folders for the three selected classes, planned selected domains, diagnostic/boundary domains, problem sets, baseline groups, smoke presets, final matrix rows, and external sketch source declarations. Final-data and matrix scripts render from this registry, and no-hardcoding tests reject embedded benchmark PDDL paths in those scripts. | Fill in local PDDL/problem sets and strict ASL rows for Gripper, Ferry, Miconic, Spanner, Childsnack, Barman, Visitall, and Delivery through the registry. TEG remains future overlay work. | Done |
...
| Feature-definable goal-dependent construction classes | Current main claim and planned main target | Strict paper-profile Blocksworld rows with selected external sketch evidence, safe feature binding, selected Layer C agenda evidence, and no runtime full-trace planner. | This class counts one selected domain name: Blocksworld. `qclear`, `qon`, and `qbw` are Blocksworld problem-family tags, not separate domains. |
...
- Feature-definable goal-dependent construction classes: Blocksworld.
