...
| N5 | Improve Layer C counterexample candidate generation beyond explicit ordering/state coverage. | G3, G7 | A held-out goal-dependency failure generates a new executable composer/module candidate and improves held-out coverage. | In progress: top-level failures after partial progress now preserve both goal-ordering and state-coverage refinement signals, with regression coverage in `test_top_level_failure_after_progress_keeps_goal_ordering_signal`; remaining work is to show the generated candidates improve held-out coverage end to end. |
