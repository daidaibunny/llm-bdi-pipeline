...
| A5 | Append query-specific temporal goals to a domain ASL library. | Implemented as stateful TEG wrapper | `append_temporal_goal_to_library` appends `+!g_query` plans for positive progress literals. Each wrapper now uses a `teg_state(Goal, State)` belief in the plan context, updates that belief after the atomic subgoal succeeds, and recurses to the same query goal. Accepting plans terminate only when `teg_state` is an accepting DFA state. |
...
| A12 | Replace raw MOOSE macro dump with compact recursive atomic modules when claiming final domain-level ASL quality. | Open | The current MOOSE Blocks snapshot is lifted but not compact: it compiles the raw MOOSE decision list into 72 `+!on(...)` macro rules. This is useful backend evidence, not the final ideal library. The target artifact should have modules such as `+!clear(X)` and `+!on(X,Y)` with recursive subgoal calls; producing that requires a validated policy/module backend or a principled compression layer, not direct raw policy emission. |
