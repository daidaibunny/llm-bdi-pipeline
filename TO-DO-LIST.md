...
| G3 | Layer C | Goal dependency handling is much stronger, but still not a complete learned composer. Generic `ready_<predicate>` gates now prevent generic `+!g` plans from ignoring selected support-agenda predecessors. The former Blocksworld all30 loop/timeout failure has been reclassified: `p21`-`p30` contain schema-level goal mutexes. A new satisfiable-large split now gives positive larger-tower evidence: 50-140 block full-tower problems solve 10/10 under both bootstrap and paper external profiles. | Broaden this beyond all-table full-tower goals: add satisfiable larger splits with nontrivial initial towers, multiple towers, and mixed `on`/safe `ontable` goals, while keeping goal-mutex diagnostics separate. | High |
...
| P3 | Blocksworld expanded paper-profile run | Most direct positive evidence. | Strict paper-profile learner-sketches run on a fixed larger train/test split, not only first20 smoke. | Done for first satisfiable larger split: generated `src/domains/blocksworld/satisfiable-large` with 10 full-tower problems from 50 to 140 blocks. Bootstrap train1 and paper external `blocks_4_on_2` both solve 10/10; paper row is `paper_profile_ready=true`. The old all30 row remains only a goal-mutex diagnostic. |
...
| N19 | Create and evaluate a satisfiable larger Blocksworld held-out split. | G3, G6 | Add a reproducible split, verify it has no schema goal mutexes, wire it into the expanded matrix, and run paper-profile evaluation. | Done: `scripts/generate_blocksworld_satisfiable_split.py` generates deterministic full-tower problems; `src/domains/blocksworld/satisfiable-large` tracks the 50-140 block split; `paper-expanded-smoke` now points its larger Blocksworld rows at this split. Reports in `tmp/domain-level-experiment-matrix/blocksworld-satisfiable-large/` show bootstrap 10/10 and paper external 10/10 with no failures. Executor was optimized to compute `ready_` derived contexts only for plans that need them, making the 140-block problem complete within the 30s per-problem budget. |
