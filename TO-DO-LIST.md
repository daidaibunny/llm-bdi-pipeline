...
| External references | Published/local MOOSE comparison complete; full C/D primary matrices received; exact serial repair handed off | The original twelve MOOSE domains use the explicitly labelled five-model coverage reported in arXiv `2511.11095v1`, Table 4; no published runtime is compared locally. The five completed four-domain Raw MOOSE runs obtain 26, 23, 23, 25, and 20 VAL-valid plans of 148, or 15.81% mean with 1.56 percentage-point sample standard deviation. All 740 portable outcomes are frozen in `paper_artifacts/gp2pl_evaluation/v1/raw_moose_extension_five_seed_summary.json`, and the generated Reported/Measured table is in the main paper. The received 20-worker LAMA/MRP+HJ and FOND4LTLf plus LAMA summaries each cover all 1,228 registered cases, but contain 9 and 46 infrastructure failures respectively. The C repair has already cached `instance-94=timeout` and `instance-95=planner_failed`; `scripts/run_external_reference_infrastructure_repairs.sh` resumes the remaining exact C set, runs the exact D set, and performs both fail-closed merges with one worker. Retain the hardware-equivalent repair runtimes for PAR-2. |
