...
| G2 | Layer B | Atomic module learning is still mostly schema/sketch candidate selection. | PDDL add-effect rules, precondition subgoal preparation, external sketch candidates, bounded progress constraints, trace slicing, last-achiever marking, trace-justification of selected atomic rules, and lifted anti-unified support patterns now exist. | Add multi-strategy module learning and recursion descent checks on top of the trace slices and anti-unified patterns. | From training traces and schemas, the system can justify each selected `+!P(...)` module and reject unsafe alternatives. | Partially done. |
