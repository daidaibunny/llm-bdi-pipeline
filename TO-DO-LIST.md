...
| Temporal experiment harness | Complete, frozen, and inserted; cross-seed manuscript update pending | The three reported ablation variants consume the same seed-0 Full GP2PL libraries and the same benchmark, bindings, and DFAs. Unprotected, Module-Return, and Balanced obtain 1,113, 1,212, and 1,228 valid traces of 1,228; transition-repair fan-out is 3, 2, and 2. The integrated final-method extension evaluates Balanced over all five independently seeded Full atomic libraries and obtains 6,140/6,140 valid traces, zero seed-sensitive cases, and identical primitive-action counts for every query. Both scopes are frozen in `paper_artifacts/gp2pl_evaluation/v1/paired_ablation_results.json`; the manuscript still needs to replace its fixed-core limitation/future-work wording with this observed robustness result. |
