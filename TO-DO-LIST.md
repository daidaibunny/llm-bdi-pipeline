...
| Check | Result |
| --- | --- |
| Full Python test suite | Blocked by pre-existing temporal appender worktree changes: current full run is `147 passed, 2 failed, 2 warnings`, with both failures expecting old context-only temporal wrappers while the dirty worktree emits `tg_state(...)`. Atomic focused suites pass. |
