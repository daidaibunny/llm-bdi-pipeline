...
| A5 | Append query-specific temporal goals to a domain ASL library. | Implemented as stateful TEG wrapper | `append_temporal_goal_to_library` appends `+!g_query` plans for positive progress literals. Each wrapper now uses a `teg_state(Goal, State)` belief in the plan context, updates that belief after the atomic subgoal succeeds, and recurses to the same query goal. Accepting plans terminate only when `teg_state` is an accepting DFA state. Append metadata now records DFA progress request diagnostics, while the ASL body still calls atomic PDDL predicate subgoals directly rather than `goal_*` descriptors. |
