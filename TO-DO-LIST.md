...
| G3 | Layer C | Goal dependency handling is much stronger, but still not a complete learned composer. Generic `ready_<predicate>` gates now prevent generic `+!g` plans from ignoring selected support-agenda predecessors. The former Blocksworld all30 loop/timeout failure has been reclassified: `p21`-`p30` contain schema-level goal mutexes. Positive larger Blocksworld evidence now includes both 50-140 block full-tower goals and 20-110 block mixed multi-tower goals with nontrivial initial towers; both solve 10/10 under the paper external profile. | Next improvement should move beyond Blocksworld positive evidence into broader domains or a learned route/ranking feature; keep goal-mutex diagnostics separate from execution-policy failures. | High |
...
| G11 | Transport-style cyclic-route and logistics modules | Partially improved. Layer B now learns a generic route-step shortest-path descent context for single-effect movement schemas, for example an action that deletes `P(entity, from)` and adds `P(entity, to)` under static edge preconditions. This supports cyclic route graphs without trace replay. Transport still remains 0/10 because full package delivery also needs a Layer C/resource agenda for vehicle/package/capacity ordering; failures now occur after route/pick-up/drop rules are available, not because cyclic route recursion is wholly unsupported. | Extend Layer C beyond atomic route movement into resource-aware logistics agendas, or keep Transport as an explicit limitation while using route-progress tests as the positive claim for the movement fragment. | High |
...
| P8 | Cyclic-route ranking/reachability | Hardest non-Blocksworld improvement. | Generic ranking/reachability feature solves single-effect movement recursion without trace replay; richer logistics/resource domains are either handled by a separate agenda method or excluded. | Done for the single-effect movement fragment: schema synthesis infers `route_step_*` contexts from PDDL movement schemas, executor derives them by static shortest-path distance decrease, recursion audit accepts only action-recursion rules guarded by route-step descent, and contract treats them as read-only contexts. Transport/Marsrover full-domain coverage remains a Layer C/resource-agenda limitation, not a route-progress limitation. |
...
| N6 | Build a broader experiment matrix. | G6, G9 | At least Blocksworld plus one non-Blocksworld domain report coverage, library size, runtime, learning audit, paper-profile readiness, and baseline comparison rows. | Done for the current paper-expanded matrix. Latest run at `tmp/domain-level-experiment-matrix/paper-expanded-smoke-latest/` produced 12 reports: Blocksworld paper external first20 20/20, satisfiable-large 10/10, satisfiable-mixed-large 10/10, all `paper_profile_ready=true`; Labworkflow refinement 2/2; Satellite train3 first10 10/10; Transport remains diagnostic 0/10; Marsrover fails before synthesis due bounded transition-system explosion. External baselines remain G9 future work. |
...
| N12 | Expand the matrix across all local PDDL domains and make long rows diagnostic-safe. | G6, G10, G11 | `paper-expanded-smoke` covers Blocksworld, Labworkflow, Transport, Satellite, and Marsrover; per-entry and per-problem timeouts prevent runaway rows; failures identify the next generic Layer B/C gaps. | Done for the current diagnostic suite. Latest `paper-expanded-smoke-latest` summary: 11 succeeded rows, 1 failed diagnostic row; Blocksworld paper-profile rows are 20/20, 10/10, and 10/10; Satellite train3 first10 is 10/10; Transport train3 first10 remains 0/10 with plan_count=28 because package-delivery/capacity-resource agenda learning is still missing; Marsrover fails before library generation because bounded transition-system exploration exceeds 20000 states. |
| N13 | Design and implement a principled cyclic-route progress feature for same-predicate movement modules. | G2, G11 | A generic PDDL domain with cyclic route facts learns a lifted `P(entity, target)` module that reaches the target without trace replay, recursive loops, or runtime full-trace planning. | Done for single-effect movement schemas: `test_schema_synthesizer_learns_route_progress_for_cyclic_movement` proves a lifted route module solves a cyclic two-hop route with `route_step_at_move(...)`; route features are metadata-backed read-only contexts, not plan heads or body subgoals. The Transport first10 row at `tmp/domain-level-experiment-matrix/transport-route-progress-smoke/` still reports 0/10 because remaining failures require package-delivery and capacity-resource agenda learning; the selected library has plan_count=28, contains `at_route_progress_via_drive`, and no longer emits unsafe `capacity_route_progress_*` rules. |
