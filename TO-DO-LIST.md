...
| AAAI paper package | Theoretical framework, source-verified citation audit, and supplement complete; paired results pending | The manuscript presents GP2PL as a certificate-carrying representation-compilation framework and AgentSpeak(L)/Jason as its evaluated realization. External definitions, prior methods, software, benchmark provenance, and baseline protocols now carry sentence-local primary citations; GP2PL claims use internal propositions, tables, appendices, and released artifacts. The standalone technical supplement contains formal assumptions, complete proofs, a theory-to-code map, the novel-data appendix, source provenance, parameters, environment, distributions, and Checklist evidence. The anonymous PDF keeps the named public repository disabled. The registered five-seed compiler, temporal-ablation, and external-reference matrices still must complete before comparative or significance claims are inserted. |
