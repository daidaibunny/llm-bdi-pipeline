...
| AAAI paper package | All current empirical floats inserted in the main paper; page pruning deferred | The main manuscript now includes the five-seed empirical figure, compact and complete five-seed atomic tables, temporal-profile table, fixed seed-0 per-domain table, exact totals, and localized Logistics/Depots failure analysis. Every empirical float uses AAAI-compatible top placement (`figure*`/`table*` or `table` with `[t]`). The standalone supplement retains compiler-freeze provenance, proofs, source map, prompt, detailed diagnostics, and reproduction commands without duplicating those floats. Paired compiler/temporal ablations and external references remain separate unfinished experiment groups; no comparative or significance claim is inserted from them. Decide how to consolidate floats only after the complete result narrative is reviewed. |
