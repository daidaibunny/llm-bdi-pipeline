...
| Atomic experiment harness | In progress | Native Evidence Adapter, Action Closure, Maximal Certified, and Full Compiler modes now share one normalized evidence fingerprint. Complete the isolated paired runner and aggregate reporting. |
| Temporal experiment harness | Native variants implemented; paired runner pending | Unprotected DFA, Certified Flat, Certified Balanced, and Completion Monitor now share one fixed atomic-library and DFA fingerprint contract. Complete aggregate execution and reporting. |
