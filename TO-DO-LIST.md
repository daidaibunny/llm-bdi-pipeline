...
| Main empirical figure | Complete from the frozen paired ablation release | `scripts/generate_aaai_figures.py --ablation-results` renders Figure 3 from all 24,560 atomic and 4,912 temporal outcomes. Panel (a) reports Evidence Only versus Full GP2PL in every domain over all five seeds; panel (b) shows the four atomic variants' coverage-size tradeoff; panel (c) reports cumulative valid temporal traces over all 1,228 paired queries. The 7.0 by 4.25 inch PNG is generated directly at 600 dpi with regular 9-pt Helvetica and colorblind-safe color plus marker/line-style redundancy. The generator recomputes paired counts and success oracles and leaves the prior PNG untouched on any provenance or data mismatch. |
