...
| AAAI paper package | Figure production briefs complete; baseline cells pending | The canonical outline specifies exact PowerPoint coordinates, text, colors, connector semantics, and captions for three conceptual figures, plus a fail-closed JSON data contract for the empirical figure. `scripts/generate_aaai_result_tables.py` validates frozen inputs and emits readable grouped result tables. Populate figures and comparison cells only from hash-locked runs. |
