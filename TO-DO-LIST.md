...
| External references | Published/local MOOSE comparison complete; C/D matrices pending | The original twelve MOOSE domains use the explicitly labelled five-model coverage reported in arXiv `2511.11095v1`, Table 4; no published runtime is compared locally. The five completed four-domain Raw MOOSE runs obtain 26, 23, 23, 25, and 20 VAL-valid plans of 148, or 15.81% mean with 1.56 percentage-point sample standard deviation. All 740 portable outcomes are frozen in `paper_artifacts/gp2pl_evaluation/v1/raw_moose_extension_five_seed_summary.json`, and the generated Reported/Measured table is in the main paper. LAMA, ENHSP MRP+HJ, and FOND4LTLf plus LAMA retain pinned native tools and independent validation; only those incomplete C/D matrices remain to run. |
