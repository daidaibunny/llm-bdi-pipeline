...
| AAAI paper package | Theoretical framework and supplement complete; paired results pending | The manuscript presents GP2PL as a certificate-carrying representation-compilation framework and AgentSpeak(L)/Jason as its evaluated realization. The standalone technical supplement contains formal assumptions, complete proofs, a theory-to-code map, the novel-data appendix, source provenance, parameters, environment, distributions, and Checklist evidence. The anonymous PDF keeps the named public repository disabled. The registered five-seed compiler, temporal-ablation, and external-reference matrices still must complete before comparative or significance claims are inserted. |
