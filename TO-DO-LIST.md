...
| Support-dependent construction goals | `blocks`, `depots` | Tests construction goals where support, stacking, clearing, or lifting relations create ordered internal atomic modules. |
...
| A9 | Materialize the six selected domains with deterministic splits. | Updated to Depots scope | `uv run python scripts/materialize_achievement_benchmarks.py` rebuilds `src/domains` with `ferry`, `miconic`, `gripper`, `logistics`, `blocks`, and `depots`. The four MOOSE domains use the official source `training/` and `testing/` split; `blocks` and `depots` use `floor(1/4 * N)` train and remaining test for support-dependent construction audits. |
...
| A13 | Keep the temporal append path compatible with the new compact atomic library. | Implemented for previous scope; Depots rerun pending | The previous six-domain smoke included `8puzzle-1tile`. The formal scope now replaces it with `depots`, so the next timestamped MOOSE-to-ASL batch must regenerate the six maintained libraries and append the first two test-query wrappers for `depots`. |
...
| Six-domain canonical ASL smoke | Previous scope result is archived evidence only: it included `8puzzle-1tile`, which is now a boundary domain rather than a selected benchmark. The next accepted six-domain smoke must cover `ferry`, `miconic`, `gripper`, `logistics`, `blocks`, and `depots`. |
