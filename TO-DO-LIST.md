...
| N9 | Keep no-hardcoding and generated-output audits current after every synthesis change. | G2, G3, G8 | `uv run pytest tests/domain_level_planning/test_no_domain_hardcoding.py -q` and relevant generated-output tests pass. | Ongoing, latest pass after paper-quality evidence restructuring and final artifact audit: no-hardcoding audit `1 passed`; final-data/Fast Downward focused checks `9 passed, 2 warnings`; full suite `292 passed, 2 warnings`. |
