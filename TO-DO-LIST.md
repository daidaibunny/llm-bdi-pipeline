...
| G3 | Layer C | Goal dependency composer is the largest research gap. | Shared-object trace ordering evidence, schema causal-interference ordering candidates, bounded state-coverage constraints, and evidence-matrix counts now exist. | Extend ordering learning with counterexample-driven failed-execution constraints and richer final-goal causal structure. | Strong goal-dependency domains such as Blocksworld produce bottom-up composer rules without domain hardcoding. | Partially done. |
