...
| A12 | Replace raw MOOSE macro dump with compact recursive atomic minimal literal modules when claiming final domain-level ASL quality. | Implemented for positive PDDL predicate seeds | `src/domain_level_planning/atomic_module_synthesis.py` uses MOOSE singleton predicates as evidence, then synthesizes compact recursive modules from PDDL action schemas. The synthesizer now performs schema-level STRIPS feasibility checks, typed action-argument compatibility, PDDL-derived `type_<type>` context guards, and typed functional fluent conflict pruning before emitting ASL. This removes invalid schema bridges such as Blocks `pick_up; unstack; stack`, removes impossible simultaneous-location branches in Miconic/Logistics, and keeps the output lifted. |
...
| Check | Result |
| --- | --- |
| Full Python test suite | `121 passed, 2 warnings` with `PYTHONDONTWRITEBYTECODE=1 uv run pytest -p no:cacheprovider -q`. |
...
| Six-domain canonical ASL smoke | Existing MOOSE readable policies under `tmp/domain_canonical_smoke_subset/` compile and append successfully for all selected domains. Final totals: `ferry 8`, `miconic 12`, `gripper 8`, `logistics 13`, `blocks 10`, and `8puzzle-1tile 10` plans including two query-wrapper plans each. Atomic audit found `0` grounded atomic arguments and no synthetic names in all six libraries. |
