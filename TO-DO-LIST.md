...
| Generated artifact hygiene | Final paper inputs no longer depend on ignored `tmp/` files. The tracked `paper_artifacts/final_paper_manifest.json` is the reproducibility contract; `tmp/paper-final-latest` is a rebuildable output. The learner-sketches Blocksworld vocabulary adapter is tracked under `src/benchmark_data/external_vocab/`, final-data runs clean their managed package directories, and standalone matrix runs remove stale managed JSON reports before writing fresh outputs. |
...
| D11 | Use a literature-backed domain-support taxonomy, not an invented "serialized goal dependency" class. | Accepted | The tracked taxonomy is `paper_artifacts/domain_support_taxonomy.json`. Main strict evidence is goal-dependent, feature-definable Blocksworld. Ferry/Gripper are control candidates, Spanner/Childsnack are ordered-resource stress candidates, Transport/Marsrover are boundary rows, and numeric/object-specific fragments are excluded. |
...
| G5 | Feature binding | Done for the declared safe DLPlan fragment: bindings cover concept, forward/reverse role, nullary, goal-aligned concept/role intersection, filtered goal-role, action-effect candidates, producer-precondition bridges, and object-free goal-distance features from a unary state predicate to its matching goal predicate through a declared binary role. Concrete object-specific, unsupported distance, and vocabulary-mismatch expressions remain structured rejections. | Future expansion must add only principled lifted bindings; concrete `c_one_of(...)` features still cannot enter a domain-level library without a verified lifting semantics. | Done |
| G6 | Validation | Done for the current paper package. The final-data protocol produces main, ablation, limitation, baseline, comparison, and LaTeX macro outputs. Main Blocksworld strict paper-profile rows solve first20, satisfiable-large, and satisfiable-mixed-large with runtime planner `none`; Labworkflow supplies the Layer C/counterexample-refinement stress because current Blocksworld no-Layer-C probes still solve 100% and therefore do not isolate that mechanism. `scripts/run_final_paper_data.py --validate-only` now checks the tracked artifact manifest, external setup commands, pinned external repository commits, matrix configs, expected table rows, required inputs, matrix summaries, baseline rows, strict-profile rows, coverage-drop ablations, runtime-planner labels, generated result macros, every referenced matrix report's current architecture contract summary, generated-output audit, coverage payload, and absence of stale unreferenced report JSON files. | Future validation can broaden the domain set or add a harder Blocksworld Layer C stress split, but the current final paper tables must describe this boundary honestly. | Done |
| G7 | Counterexample refinement | Done for the declared positive-conjunctive failure classes: missing atomic modules, missing composer contexts, primitive positive-precondition repairs, explicit goal ordering, state coverage, recursive-loop, and step-limit failures now either generate selector-consumable Layer B/Layer C constraints or structured diagnostics. Termination failures keep diagnostic constraints and add atomic-progress/state-coverage companion constraints when supported missing goals remain. | Future unsupported failure classes may be added, but current held-out failures inside the supported fragment no longer stop at diagnosis only. | Done |
...
| G12 | Domain-support boundary | Done for current paper framing: the final paper now uses prior terms from MOOSE, policy sketches, generalized policies, policy reuse, and lifted decision-list/program-synthesis baselines. The support class is "goal-conditioned, feature-definable, terminating modular policy sketches with bounded sketch width and safe lifted feature bindings." | Next implementation expansion should target one control family domain such as Ferry/Gripper and one standard ordered-resource stress domain such as Spanner/Childsnack before broadening claims. | Done |
...
| P7 | Feature-binding expansion | Only after core paper claim is stable. | Add principled bindings only for features with verified lifted semantics; otherwise keep structured rejection. | Done for current paper scope: external backend reports include a machine-readable `feature_binding_contract`, per-policy summaries, unsupported rejection reason counts, and the new object-free goal-distance binding. Concrete object-specific features remain rejected rather than guessed. |
...
| Goal-dependent feature-definable construction domains | Yes | Strict paper-profile Blocksworld rows with selected external sketch evidence, safe feature binding, selected Layer C agenda evidence, and no runtime full-trace planner. | Layer C necessity must be argued through mechanism-stress rows if Blocksworld no-Layer-C also solves the current split. |
| Goal-independent control domains | Candidate, not current main claim | Ferry or Gripper should pass the same ASL-library protocol before being counted. | MOOSE/PG3 success on these domains is related-work evidence, not our method's coverage. |
| Ordered-resource stress domains | Candidate/stress, not current main claim | Spanner or Childsnack should be added as standard benchmark stress after Labworkflow. | Labworkflow is useful mechanism isolation but should not be presented as a standard benchmark family. |
...
| Reproducibility | Let the final numbers be regenerated. | Commit hash, command lines, train/eval split files, resource limits, timeout settings, backend pins, and generated LaTeX macros. | A single documented command sequence regenerates the result directory and `latex_code/aamas_method_paper/sections/result_macros.tex`. |
...
| 6. Comparison and macros | `tmp/paper-final/comparison.json` and `latex_code/aamas_method_paper/sections/result_macros.tex` | `uv run python scripts/compare_domain_level_experiments.py ... --latex-macros-output ...` | The paper must import generated macros instead of manually transcribing final values. |
...
| N9 | Keep no-hardcoding and generated-output audits current after every synthesis change. | G2, G3, G8 | `uv run pytest tests/domain_level_planning/test_no_domain_hardcoding.py -q` and relevant generated-output tests pass. | Ongoing, latest full suite after final paper reproducibility cleanup: `297 passed, 2 warnings`. |
...
| N14 | Rewrite the method plan around the simplified core contribution before adding more functionality. | G1, G2, G3 | The architecture contract and tests distinguish paper core method from implementation safeguards; the next implementation task can be justified by the core method, not by ad hoc failure patching. | Done for the paper-method draft: `latex_code/aamas_method_paper/main.tex` and `sections/method.tex` define the AAMAS-formatted formal algorithm, candidate language, selector, compiler semantics, evaluation protocol, and exclusions. The guarantee is now written as Proposition 1 with assumptions, claim, and proof sketch for bounded-class soundness. Verified with `latexmk -pdf main.tex`. |
| N15 | Expand the method draft into a full evaluation-ready paper skeleton. | G1, G6, G9 | Add related work, experiment tables, ablation interpretation, final claim checklist, and result-import macros without changing the fixed core algorithm. | Done for the current paper package: `sections/evaluation.tex` now imports generated result macros and separates strict paper-profile Blocksworld rows from mechanism-stress Labworkflow rows and boundary Transport/Marsrover rows. It states that Labworkflow is the current Layer C/refinement stress row and that Blocksworld no-Layer-C is not used as mechanism-necessity evidence because it remains 100% on current splits. |
...
| N21 | Build the final paper data package. | G6, G9 | Produce `tmp/paper-final/` with backend audit logs, main library reports, baseline JSON rows, ablation reports, limitation diagnostics, `comparison.json`, generated LaTeX result macros, and a copied artifact manifest. | Done for the current final-data protocol. Tooling: `scripts/run_final_paper_data.py` reads the tracked `paper_artifacts/final_paper_manifest.json`, writes fixed main/ablation/limitation configs, copies the manifest into the output package, runs backend audits, generates baselines, runs matrices, writes `comparison.json`, and emits `latex_code/aamas_method_paper/sections/result_macros.tex`. Fresh non-validate runs clean managed output directories before regeneration, preventing stale report carryover. Result macro generation includes split-specific baseline macro identifiers, rejects duplicate macro prefixes, and converts digits to words so generated commands are valid LaTeX control sequences. The learner-sketches Blocksworld vocabulary adapter is now a tracked input under `src/benchmark_data/external_vocab/`, not an ignored `tmp` artifact. The validator `uv run python scripts/run_final_paper_data.py --output-dir tmp/paper-final-latest --validate-only` passes with manifest-level checks including current architecture-contract consistency for every referenced report, stale report rejection, expected row equality, matrix-config equality, required inputs, and the required five-problem Labworkflow Layer C stress rows. Latest regenerated package reports 10 library rows, 10 baseline rows, and 20 paper-table rows. |
| N25 | Remove stale generated-artifact dependencies and clean obsolete local outputs. | G6, G9 | Final paper configs and matrix presets must not depend on ignored `tmp/` inputs; repeated matrix runs must not leave stale JSON reports; tests must not write repository-level temporary fixtures; old local `tmp/`, bytecode, and build-cache artifacts should be removed after regenerating the current final package. | Done: stable vocabulary adapter added under `src/benchmark_data/external_vocab/`; tracked artifact manifest added under `paper_artifacts/`; final-data and matrix presets now use tracked inputs and manifest contracts; tests assert external vocabulary adapter paths are tracked and not under `tmp/`; standalone matrix runner now removes stale generated JSON while preserving an explicitly supplied config file; schema-synthesis tests now use pytest `tmp_path` instead of `tmp/schema-synthesis-tests`; obsolete local `tmp/`, `tests/generated/`, bytecode, pytest cache, `.DS_Store`, and AAMAS build artifacts were cleaned, then the current `tmp/paper-final-latest` package was regenerated and validated with manifest-level checks. |
| N22 | Add or select a goal-dependency stress split for Layer C ablation. | G3, G6 | On the same split, the full method succeeds and the no-Layer-C variant either fails, times out, or emits a clear missing-composer/unsafe-composer diagnostic. | Done via scalable Labworkflow goal-dependency stress (`p02`-`p06`, train only `p01`). Full counterexample-refinement row solves `5/5`; `no_layer_c_with_refinement_labworkflow_stress` solves `0/5`; `no_counterexample_refinement_labworkflow_stress` solves `0/5`; both failing rows report `no applicable plan for !reagent_logged(r1): 5`. Pure goal-ordering refinement now stays constraint-only, so larger held-out failures do not force bounded transition-system exploration. This is the final Layer C/counterexample stress evidence; Blocksworld no-Layer-C remains 100% on current splits and should be described as not stressing that mechanism. |
...
| N26 | Finalize paper gaps 3-6. | G1, G6, G9 | External artifact setup is machine-checked, citations are verified and corrected, paper narrative states the bounded contribution cleanly, and the AAMAS build has no avoidable project-level warnings. | Done. Manifest records external setup/preflight commands plus learner-sketches, h-policy-learner, d2l, MOOSE, MOOSE benchmarks, and VAL commit pins; final-package validation checks declared pins and the tracked `sections/result_macros.tex` plus the embedded `main.tex` macro block. Citation metadata was verified; Clingo year and MOOSE pages were corrected, and Frances et al. keeps the official AAAI proceedings title. Paper prose now distinguishes executable ASL libraries from planner wrappers and explains Layer B versus Layer C. Verification: final package regenerated and `--validate-only` passed with `checks=298`; `uv run pytest -p no:cacheprovider -q` passed with `297 passed, 2 warnings`; `latexmk -g -pdf -interaction=nonstopmode -halt-on-error -outdir=build main.tex` produced the six-page PDF with no undefined references or BibTeX warnings. The remaining `\ifx` message is template-level: a minimal `\documentclass[sigconf,anonymous]{aamas}` document reproduces it, so it is not a project-generated macro/citation warning. |
