...
| A12 | Replace raw MOOSE macro dump with compact recursive atomic minimal literal modules when claiming final domain-level ASL quality. | Implemented for all producible positive PDDL fluents with Clingo branch selection | `src/domain_level_planning/atomic_module_synthesis.py` uses MOOSE singleton predicates as seed-goal evidence, then synthesizes compact recursive modules from PDDL action schemas for every predicate that appears in a positive action effect. Static predicates are detected schema-theoretically by absence from add/delete effects and remain context-only. The synthesizer performs schema-level STRIPS feasibility checks, typed action-argument compatibility, and typed functional fluent conflict pruning before candidate generation. Final branch selection is Clingo/ASP-backed obligation coverage with lexicographic minimization over selected branch count, context literal count, and body step count. PDDL typing is internal only; final ASL must not emit `type_*` fluents. Blocks now emits `+!holding(X)`, `+!handempty`, and `+!ontable(X)` modules in addition to `+!on(X,Y)` and `+!clear(X)`. |
