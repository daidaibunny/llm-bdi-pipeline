...
| Main empirical figure | Generator complete; final data pending | `scripts/generate_aaai_figures.py` renders the two-figure paper's empirical Figure 2 from one complete `paired_results.json`. The 7.0 by 3.0 inch vector output uses regular-weight embedded fonts, print-safe color/shape/line redundancy, and shared temporal checkpoints. It rejects dirty, incomplete, or unpaired matrices and writes no release PDF until the registered five-seed atomic and four-variant temporal matrix is complete. |
