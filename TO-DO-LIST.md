...
| G6 | Validation | Current validation is smoke/bounded, not yet a broad paper experiment suite. Reports now include machine-readable mechanism status for ablation rows, so comparison tables distinguish external sketch evidence, counterexample refinement, offline synthesis traces, Layer C ordering, and paper-profile gating instead of relying only on labels. The first explicit no-Layer-C-ordering Blocksworld first20 ablation solves 20/20 after filtering 9 ordering rules, so first20 alone does not prove the necessity of the ordering mechanism. | Multiple IPC-style domains, train/test scaling, completed ablation runs, external baselines, failure analysis, and reproducible tables. | High |
...
| G11 | Transport-style cyclic-route and logistics modules | Improved for the typed-overloaded carrier/resource fragment. Layer B now learns generic route-step shortest-path descent for single-effect movement schemas, and schema synthesis generates causal-chain atomic modules for typed-overloaded effect predicates such as `at(carrier, location)` versus `at(package, location)`. Shared static preconditions can bridge resource variables, resource predicates such as capacity are not emitted as independent causal-chain goals, and current-resource priority composer rules deliver already-held target objects before another goal consumes the same carrier. Transport train3 first10 now solves 10/10 from problem initial states with no runtime full-trace planner. Bounded all-reachable-state validation still reports mid-state counterexamples, so this is not yet a complete arbitrary reachable-state logistics policy. | Decide whether to strengthen the Transport claim with mid-state repair/counterexample refinement, or report the current fragment as initial-state delivery coverage plus bounded-validation limitations. | High |
...
| N6 | Build a broader experiment matrix. | G6, G9 | At least Blocksworld plus one non-Blocksworld domain report coverage, library size, runtime, learning audit, paper-profile readiness, and baseline comparison rows. | Done for the current paper-expanded matrix infrastructure. Latest refreshed run at `tmp/domain-level-experiment-matrix/paper-expanded-smoke-latest/` produced 12 succeeded rows and 1 diagnostic failed row: Blocksworld paper-profile first20 20/20, no-Layer-C-ordering first20 20/20 with 9 ordering rules filtered, satisfiable-large 10/10, satisfiable-mixed-large 10/10; Labworkflow 2/2; Satellite first10 10/10; Transport train3 first10 10/10 with plan_count 28 after current-resource priority; Marsrover remains 0/10 due training transition-system exploration exceeding 20000 states. Experiment and comparison reports now include mechanism status for ablation rows. External baselines remain G9 future work. |
...
| N12 | Expand the matrix across all local PDDL domains and make long rows diagnostic-safe. | G6, G10, G11 | `paper-expanded-smoke` covers Blocksworld, Labworkflow, Transport, Satellite, and Marsrover; per-entry and per-problem timeouts prevent runaway rows; failures identify the next generic Layer B/C gaps. | Done for the refreshed diagnostic suite. Latest `paper-expanded-smoke-latest` summary: 11 succeeded rows, 1 failed diagnostic row; Blocksworld paper-profile rows are 20/20, 10/10, and 10/10 with `paper_profile_ready=true`; Transport train3 first10 is 10/10; Satellite train3 first10 is 10/10; Marsrover fails before library generation because bounded transition-system exploration exceeds 20000 states. |
