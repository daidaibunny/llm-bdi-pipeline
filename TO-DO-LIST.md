...
| AAAI paper package | Official AAAI-27 format and four-table main-paper structure complete; conceptual artwork pending | The manuscript uses the official `aaai2027.sty` and `aaai2027.bst` files without legacy font overrides. The main paper reserves Figure 1 as a first-page single-column framework overview, Figure 2 as a page-2-top full-width policy-lifting example, and Figure 3 as the five-seed empirical result. Its four core tables are the supported fragment, certified candidate language, compact five-seed atomic summary, and temporal-profile summary. Raw domain-by-seed counts and the fixed seed-0 temporal input table are generated into the Technical Supplement. Figures 1 and 2 remain `\IfFileExists` placeholders until the colleague-produced vector PDFs arrive. Every float uses `[htbp]`, captions sit below content, and no manual page break or float-queue override is used. Paired compiler/temporal ablations and external references remain unfinished experiment groups; no comparative or significance claim is inserted from them. |
