...
| G9 | Counterexample refinement | Refinement hooks exist but are not a full learning loop. | Counterexample problem files add transition-progress and state-coverage constraints; held-out failures are classified into lifted Layer B/Layer C refinement records; primitive precondition failures now report failing primitive action, lifted missing goals, and lifted missing preconditions; primitive repair constraints are fed into later synthesis rounds and become selector hard groups when existing prepare candidates match; unmatched repair constraints are reported as rejected binding diagnostics with available capabilities and rejection reason instead of guessed or silently ignored; refinement traces include a machine-readable summary of convergence, held-out failures, added counterexamples, repair constraints, and constraints grouped by failure kind, target layer, and constraint type. | Connect more failure classes to generated candidate rules, and synthesize missing candidates when no existing rule matches a failure classification. | A failed held-out problem can automatically refine the library and improve validation coverage, with report summaries showing what changed. | Partially done. |
