...
latest ready-agenda rerun: `tmp/domain-level-experiment-matrix/blocksworld-paper-external-on2-all30-ready-agenda.json`
diagnosis with 30s per-problem timeout: `p21` still fails with recursive loop
  on `!g`; `p22`-`p30` exceed the diagnostic timeout. This is not an
  external-policy synthesis failure; it is the current larger-scale Layer C /
  execution-policy gap for deeper Blocksworld towers. The new generic
  `ready_<predicate>` context gate is necessary runtime agenda plumbing but is
  not sufficient to solve the all30 scaling gap.
```
...
| G3 | Layer C | Goal dependency handling is much stronger, but still not a complete learned composer. Generic `ready_<predicate>` gates now prevent generic `+!g` plans from ignoring selected support-agenda predecessors, but Blocksworld all30 remains 20/30 and still exposes `!g` loop/timeout failures on deeper towers. | Goal-agenda and composer synthesis handle goal-dependent domains robustly, with acyclic support agendas, explainable delete-threat diagnostics, counterexample-driven new composer candidates, and scalable execution on larger held-out Blocksworld instances. | High |
