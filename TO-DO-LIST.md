...
| External references | Existing matrices frozen; local TIDE path implemented | The original twelve MOOSE domains use the explicitly labelled five-model coverage reported in arXiv `2511.11095v1`, Table 4; no published runtime is compared locally. The five four-domain Raw MOOSE runs obtain 26, 23, 23, 25, and 20 VAL-valid plans of 148. On the identical 740 added-domain seed--case evaluations, the main paper reports Raw MOOSE evidence at 117/740 and Full GP2PL at 720/740. The complete repaired matrix obtains 591/868 for LAMA and 253/360 for MRP+HJ; the direct FOND4LTLf matrix supports 492/1,228 inputs and validates 298, with 736 explicit unsupported cases. All 2,456 existing external records remain frozen in `paper_artifacts/gp2pl_evaluation/v1/external_reference_results.json`. The pinned official TIDE feedback/heuristic/cache/LAMA runtime now uses reversible delimiter-safe PDDL alpha-renaming, parent-first type normalization, decoded plan projection, and the common VAL/DFA validation path. All 868 Boolean cases pass static normalization and one real case from each of the 12 applicable domains passes the complete validation path; a complete same-scope TIDE matrix remains pending and must not be claimed before it is frozen. |
