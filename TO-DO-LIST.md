...
| AAAI paper package | Three-figure layout reserved; conceptual artwork pending | The main manuscript reserves Figure 1 as a first-page single-column framework overview, Figure 2 as a page-2-top full-width policy-lifting example, and Figure 3 as the existing five-seed empirical result. Figures 1 and 2 use `\IfFileExists` placeholders that preserve their final dimensions until the colleague-produced vector PDFs arrive; no synthetic preview graphic is treated as paper artwork. Every float uses `[htbp]`, captions sit below the visual, and no manual page break or float-queue override is used. Paired compiler/temporal ablations and external references remain separate unfinished experiment groups; no comparative or significance claim is inserted from them. |
