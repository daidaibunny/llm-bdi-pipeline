...
| AAAI paper package | Current figures and tables repaired and visually verified; page pruning deferred | The main manuscript includes the five-seed empirical figure, compact and complete five-seed atomic tables, temporal-profile table, fixed seed-0 per-domain table, exact totals, and localized Logistics/Depots failure analysis. Every main and supplement figure/table float uses `[htbp]`; captions sit below their visual; table and figure text is at least 9 pt; no manual page break or float-queue override is used. The result floats now precede the conclusion and sit beside their substantive discussion. Paired compiler/temporal ablations and external references remain separate unfinished experiment groups; no comparative or significance claim is inserted from them. Defer page-count reduction until the complete result narrative is reviewed. |
...
| External references | Portable runtime fixes complete; full matrices pending | Raw MOOSE, LAMA, ENHSP MRP+HJ, and FOND4LTLf plus LAMA use pinned native tools, common resource guards, concise per-case logs, and independent validation. Setup rejects a macOS Java launcher without a JDK, verifies the pinned MONA version despite its nonzero version-only exit, passes MONA explicitly into internal DFA validation, pins VAL, serializes the non-reentrant MOOSE-hosted LAMA runtime, and separately serializes FOND4LTLf compilation because pinned `ltlf2dfa` shares `automa.mona`. Rerun the registered full matrices from new output roots before filling paper cells. |
...
| Main empirical figure | Complete from frozen five-seed Full GP2PL results | `scripts/generate_aaai_figures.py --five-seed-results` renders Figure 2 from the frozen 6,059/6,140 five-seed result, the verified raw runner aggregate hash, and five hash-locked Jason summaries. Panel (a) reports every domain; panel (b) follows the MOOSE-style absolute-time convention over all three predeclared benchmark groups, without selecting domains. The 7.0 by 4.25 inch vector PDF uses embedded Helvetica, a 9-pt minimum text size, grayscale plus marker/line-style redundancy, and a 1,800-second deadline. The generator recomputes counts, validates VAL success and `run_seconds`, and leaves the prior PDF untouched on any provenance or data mismatch. The separate `--paired-results` mode remains for the pending four-variant ablation. |
...
- External-reference setup now probes a working Java runtime and MONA binary,
  ...
  uses global loop devices and a shared `/work/out` directory. FOND4LTLf
  compilation uses a distinct host-wide lock because pinned `ltlf2dfa` 1.0.2
  writes every process's DFA input through one package-local `automa.mona` file.
  Scientific planner failures remain resumable results, while infrastructure
  failures are retried by `--resume`. FOND4LTLf normalization removes a
