...
| AAAI paper package | Official AAAI-27 format and evidence-complete main-paper table structure in progress | The manuscript uses the official `aaai2027.sty` and `aaai2027.bst` files without legacy font overrides. Its abstract and introduction now follow a problem--gap--method--guarantee--result cadence, introduce Blocks World in plain language before symbolic notation, and defer certificate inventories to the method. Evaluation prose now reports scientific aggregates, seed variation, failure concentration, causal interpretation, and claim boundaries; per-seed raw numerators, timeout/exit status, worker settings, hashes, run records, challenge lists, and individual case identifiers remain in the Technical Supplement. Prose and software names use the Author Kit's roman text, literal code uses Courier, and named mathematical summaries and relations use upright serif notation. Its notation separates the certified atomic module core `M_D`, per-query plan set `Q_q`, and sole maintained library version `L_D^[k]`, with append update `L_D^[k+1] = L_D^[k] union Q_q`. The main paper now summarizes the witness-backed TEG benchmark construction, including the original-goal exclusion, legal rollout mining, typed lifting, balanced deterministic selection, and 1,228-to-475 complete-input deduplication. Technical Supplement Sec. 4.1 gives the complete algorithm, five profile contracts, registered bounds, concrete example, and public/private audit boundary without shifting the frozen prompt and provenance sections. Figure 1 is the first-page single-column representation bridge, Figure 2 is the page-2-top atomic-core construction example, and Figure 3 is the paired atomic-and-temporal ablation result. The supported-fragment, certified-candidate, compact five-seed atomic, temporal-profile, paired-ablation, and external-reference tables remain in the main paper until later page-budget editing. Raw domain-by-seed counts and the fixed seed-0 temporal input table are generated into the Technical Supplement. Full-width Figures 2 and 3 use standard page-top `[t!]` floats; captions sit below content, and no manual page break or float-queue override is used. |
| Evaluation result order | Complete | The main paper now presents paired component ablations first, full-system atomic and temporal results second, and scope-separated external planning references last. Figure 3 is declared at the Evaluation opening so it visually precedes the paired discussion, and a pre-Conclusion float barrier keeps every result table ahead of the conclusion and references. |
| Manuscript prose and captions | Submission-quality audit complete; page-budget editing deferred | The abstract, introduction, foundations, related work, method, evaluation, limitations, conclusion, and technical proof/reporting text now use one conditional claim boundary. Atomic inference aggregates five seeded outcomes within held-out case identifiers before an exact sign test; temporal contrasts report discordant counts and domain/input concentration without a case-level p-value. The results state the 10.42-point atomic and 9.28-point temporal effects, the fivefold offline compile-cost tradeoff, the Balanced controller-size/fan-out tradeoff, the 720/740 versus 117/740 same-scope added-domain contrast, and the concentration of 113/114 temporal gains in four numeric domains. Captions define denominators, validation criteria, aggregation, units, PAR-2, and scope. A dedicated Limitations section now records the only instantiated evidence provider, incomplete candidate grammar, type-level closure boundary, short-horizon witness construction, controlled-language distribution, fixed seed-0 temporal core, and incomplete PDDL--LTLf strategy synthesis. The current PDFs compile cleanly; shortening to the conference page budget remains a separate final editorial pass. |
...
| Temporal experiment harness | Complete, frozen, and inserted | The four variants consume the same-run seed-0 Full GP2PL libraries and identical benchmark, binding, and DFA hashes. Unprotected, Flat, Balanced, and Module-Return obtain 1,113, 1,227, 1,227, and 1,211 valid traces of 1,228; corrected transition-repair fan-out is 3, 3, 2, and 2. |
| External references | Complete, repaired, frozen, and inserted into the manuscript | The original twelve MOOSE domains use the explicitly labelled five-model coverage reported in arXiv `2511.11095v1`, Table 4; no published runtime is compared locally. The five four-domain Raw MOOSE runs obtain 26, 23, 23, 25, and 20 VAL-valid plans of 148. The complete repaired C matrix obtains 591/868 for LAMA and 253/360 for MRP+HJ; the complete D matrix supports 492/1,228 temporal inputs and validates 298, with 736 explicit unsupported cases. Its 9 and 46 exact infrastructure-only repairs preserve hardware-comparable PAR-2. All 2,456 C/D records are frozen without machine-local paths in `paper_artifacts/gp2pl_evaluation/v1/external_reference_results.json`, and the generated external-reference table is consumed by the main paper. |
...
| Main empirical figure | Complete from the frozen paired ablation release | `scripts/generate_aaai_figures.py --ablation-results` renders Figure 3 from all 24,560 atomic and 4,912 temporal outcomes. Panel (a) reports all four atomic variants for every domain where paired coverage differs and transparently aggregates the 11 identical 100% domains; panel (b) uses all 385 seed--instance pairs in the registered 12--50-block Tower stress series; panel (c) reports cumulative valid temporal traces over all 1,228 paired queries. The 7.0 by 2.833 inch, 4,200 by 1,700 pixel PNG is generated directly at 600 dpi with regular 9-pt Helvetica and color-independent numeric/marker/line encodings. The generated paired tables use bold for tied best results and blue bold for the selected configuration, while disjoint external scopes remain unranked. The generator recomputes paired counts and success oracles and leaves the prior PNG untouched on any provenance or data mismatch. |
