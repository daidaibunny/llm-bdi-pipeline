...
seven-page limit. The main paper therefore contains Figure 1, the five-seed
empirical Figure 2, the algorithm, the certificate table, and all four generated
result tables: compact and complete five-seed atomic tables plus temporal-profile
and fixed seed-0 per-domain tables. Page-budget pruning is a later editorial
step and must not silently remove evidence while the result narrative is still
being checked. Worked policy-lifting and DFA-controller diagrams remain
reserved supplementary assets. The Technical Supplement retains proofs,
provenance, failure mechanisms, and reproduction detail without duplicating the
four empirical tables or Figure 2.

### Figure Production Handoff Contract

Figure 1 and the planned supplementary method diagrams are conceptual. Their
labels and examples below come from the implemented architecture and PDDL
schemas, not from estimated experimental measurements. Main Figure 2 is
empirical:
...
- Main Figure 1 uses a 13.333 by 5.000 inch PowerPoint export canvas.
  ...
  Main Figure 2 is generated by Matplotlib at exactly 7.0 by 4.25 inches. Keep a
  white opaque background.
- Use Arial 18 pt for normal labels, Arial Bold 20 pt for panel headings, and
  Courier New 18 pt for PDDL, AgentSpeak, formulas encoded as text, and artifact
  field names. These become approximately 9.5--10.5 pt after scaling to AAAI
  `\textwidth`.
...
- Main Figure 2 uses regular-weight Helvetica throughout, with every panel
  ...
  spines, retain only light-gray major grid lines, and use grayscale with
  redundant marker and line-style encodings.
...
- At `x=95.7,y=72,w=1.3,h=24`, draw a right-facing green brace joining
  ...
  `x=96.9,y=91,w=3,h=5` in 18 pt Arial. Nothing may extend beyond `x=100`.
...
### Figure 2: Five-Seed Empirical Evidence
...
- Use Helvetica, embedded as TrueType in the PDF, with a 9-pt minimum for every
  ...
  remain. Use grayscale with redundant marker and line-style encodings. Stars
  are not used: in empirical figures they commonly
...
> **Figure 2: Five-seed held-out execution.** Panel (a) gives per-domain
...
The main Evaluation source contains the empirical `figure*[htbp]` placement before
...
The current main paper inserts `fig2_evaluation.pdf`. The planned supplementary
conceptual assets remain optional until delivered and validated.
...
- The final empirical figure must be regenerated by a checked-in script from
  ...
  acceptable for main Figure 1 and Supplementary Figures S1--S2, but not for
  manually placing result points in main Figure 2.
...
7. the corresponding quantitative Abstract, Introduction, Evaluation,
   failure-boundary explanation, and Conclusion text.

The remaining submission tasks are:

1. completed registered atomic and temporal ablation runs, plus the separately
