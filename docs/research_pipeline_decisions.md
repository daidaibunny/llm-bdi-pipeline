...
`scripts/generate_aaai_figures.py` from the complete frozen
`paired_ablation_results.json`; it never joins values from a separate historical
run. Its atomic matrix includes every domain on which any of the four paired
variants differs and aggregates the remaining domains only when their four
coverages are identical. Its scaling panel uses all 385 seed--instance pairs in
the registered Blocksworld Tower size series, the corpus's monotonic size stress
family, rather than a selected instance. Its temporal panel uses all registered
queries as the cumulative-coverage denominator. Jason failure, validation
failure, and timeout therefore remain below each final curve endpoint. Dirty
...
`validated_evidence_adapter` is `Evidence Only`. The release graphic is a 7.0 by
2.833 inch, 600-dpi opaque PNG. Printed cell values, markers, and line styles
make color redundant; no star or bold data mark implies untested significance.
...
from LaTeX cells. The three fixed panels report (i) all four atomic variants on
every domain where their paired coverage differs, with the identical domains
aggregated transparently, (ii) cumulative time-to-valid coverage for all 385
Blocksworld Tower seed--instance pairs, and (iii) cumulative valid temporal
traces for all four controller variants over all 1,228 queries. The generator
revalidates pairing and the success oracle before writing the 7.0 by 2.833 inch,
4,200 by 1,700 pixel PNG at 600 dpi.
...
Three temporal scopes are distinct. `Phi_syn` is the literal-negation grammar
...
`!(F(a0))` and `!!a0` are outside `Phi_syn`. `Phi_bench` is the family generated
...
temporal soundness is conditional on `Phi_cert`, and the reported experiment is
over `Phi_bench`.
