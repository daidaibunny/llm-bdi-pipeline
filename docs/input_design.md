...
The evaluated family `Phi_bench` contains proper TEGs instantiated from exactly
five registered structures:
...
The fixed downstream execution result records that all 1,228 released
problem-level bindings obtained an explicit Jason
...
the released bindings and supplied atomic libraries; it is not additional
