...
The colleague combines each row with
`domains/<domain>/catalog.json`, calls the language model, and writes the
predicted LTLf manifest. That predicted manifest is a separate artifact and is
not generated by `build_temporal_nl_manifests.py`.
...
The package may depend on `utils.pddl_parser`. It MUST NOT depend on generalized
planning backends, atomic-module synthesis, query wrappers, Jason, or VAL.
